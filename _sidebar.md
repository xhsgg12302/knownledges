* DOCS

    - [BASE](docs/base/)
        + [OBJECT ORIENTED](docs/base/OO.md)
        + [IO STREAM](docs/base/IO.md)
        + [NETWORK](docs/base/network.md)
        + [THREAD](docs/base/thread.md)
        + [COLLECTION](docs/base/collection.md)
        + [EXCEPTION](docs/base/exception.md)
        + [ADVANCE](docs/base/advance.md)
        + [AQS](docs/base/AQS.md)
		
    - [DESIGN PATTERN](docs/design-pattern/dp.md)
    - [JVM](docs/jvm/jvm.md)
    - [REDIS](docs/redis/redis.md)
    - [ZOOKEEPER](docs/zookeeper/zk.md)
    - [NETTY](docs/netty/netty.md)
    - [ELASTICSEARCH](docs/es/es.md)
    - [TOMCAT](docs/tomcat/tomcat.md)
    - [MYBATIS](docs/mybatis/mybatis.md)
    - [SPRING](docs/spring/spring.md)
    - [SPRINGMVC](docs/springmvc/springmvc.md)
    - [SPRINGBOOT](docs/springboot/springboot.md)
    - [SPRINTCLOUD](docs/springcloud/springcloud.md)

    - [MYSQL](docs/mysql/README.md)
        + [INSTALL](docs/mysql/install.md)
        + [LEARNING_DOC](docs/mysql/mysql.md)
        
    - [MQ](docs/mq/)
        + [ACTIVEMQ](docs/mq/activemq/acmq.md)
        + [KAFKA](docs/mq/kafka/kafka.md)

    - [RPC](docs/rpc/)
        + [GOOGLE GRPC](docs/rpc/gRpc/gRpc.md)
        + [APACHE DUBBO](docs/rpc/dubbo/dubbo.md)
        + [BAIDU BRPC](docs/rpc/brpc/brpc.md)
        + [SERIALIZED](docs/rpc/serialized/)
            - [JAVA BUILD_IN（JDK)](docs/rpc/serialized/JDK_build_in.md)
            - [JSON/XML](docs/rpc/serialized/jsonOrxml.md)
            - [GOOGLE PROTOBUF](docs/rpc/serialized/protobuf.md)
            - [APACHE AVRO](docs/rpc/serialized/avro.md)
            - [APACHE THRIFT](docs/rpc/serialized/thrift.md)
            - [HESSION](docs/rpc/serialized/hession.md)
            - [FST](docs/rpc/serialized/fst.md)
            - [KRYO](docs/rpc/serialized/kryo.md)

    - [DISTRIBUTED(DSTB)](docs/dstb/)
        + [TRANSCATION](docs/dstb/transcation.md)
        + [LOCKS](docs/dstb/lock.md)
        + [SCHEDULE](docs/dstb/schedule/)
            - [QUARTZ](docs/dstb/schedule/quartz.md)
            - [XXL-JOB](docs/dstb/schedule/xxl-job.md)

---
* DevOps
    - [OS](docs/os/)
        + [SHELL](docs/os/shell/command.md)
    - [NGINX](docs/nginx/nginx.md)
    - [DOCKER](docs/docker/docker.md)
    - [GIT](docs/git/git.md)
    - [MAVEN](docs/maven/maven.md)
    - [K8S](docs/k8s/k8s.md)

---
* ALGORITHM
    - [SORT](docs/algorithm/sort.md)
    - [TREE](docs/algorithm/tree.md)
    - [LINKED](docs/algorithm/linked.md)
    - [ARRAY](docs/algorithm/array.md)
    - [STACK](docs/algorithm/stack.md)
    - [GRAPH](docs/algorithm/graph.md)

---
* OTHER
    - [DOCSIFY_DOC](https://docsify.js.org/#/zh-cn/)
    - [REVIEW](docs/jvm/review-preparement.md)
    - [HR_PROBLEM](docs/hr/hr.md)
    - [CTO_PROBLEM](docs/hr/cto.md)

