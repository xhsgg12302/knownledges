* ## Intro(XML Schema)
    ...
    + ### Schema约束
        ...
        <!-- 引入 -->
        <persons
                xmlns="http://docs.wtfu.site/person.xsd"
                xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                xsi:schemaLocation="http://docs.wtfu.site/person.xsd person.xsd"
                >
        ...
        - #### 编写规则
            ...
            1. `xsd`文件的根标签为`schema`.
            2. 在`schema`中定义属性 
                <br>2.1 `xmlns="http://www.w3.org/2001/XMLSchema"`，元约束。
                <br>2.2 `targetNamespace="http://docs.wtfu.site"`，指定当前这个schema文件的名称空间。
                <br>2.3 `elementFormDefault="qualified"`，表示当前文件质量良好。
            3. 如果是一个简单元素，直接在`<element>元素中定义属性即可`。
            4. 如果是一个复杂元素，需要增加子元素`<complexType>`并且在其中增加`<sequence>`和属性`<attribute>`。
            
        - #### 引入方式

            1. 通过`:`冒号的方式起别名，并且指定通过`xsi:schemaLocation`属性的方式定义资源位置。通过URL一一对应。 

        - #### 参考引用

            1. http://www.springframework.org/schema/beans/spring-beans-3.1.xsd

                <details><summary>spring-beans-3.1.xsd</summary>
            ...
            2. https://github.com/12302-bak/idea-test-project/blob/master/_4_springmvc/src/main/resources/spring-core.xml
